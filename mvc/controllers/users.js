...
const addDetailsToComments = function (posts) {
...
const generateFeed = function ({ payload }, res) {
    ...
    let myPosts = new Promise(function (resolve, reject) {
        User.findById(payload._id, '', { lean: true }, (err, user) => {
            ...
            if (!user) {
                return res.statusJson(404, { message: 'User does not exist.' });
            }
            addToPosts(user.posts, user);
            posts.push(...user.posts);

            user.friends = user.friends.filter((val) => !user.besties.includes(val));
            resolve(user);
        });
    });

    let getPostsFrom = function (usersArray, maxAmountOfPosts, arrayOfPosts) {
        return new Promise(function (resolve, reject) {
            User.find({ _id: { $in: usersArray } }, 'name profile_image posts', { lean: true }, (err, users) => {
                if (err) {
                    return res.statusJson(400, { err: err });
                }
                for (user of users) {
                    addToPosts(user.posts, user);
                    arrayOfPosts.push(...user.posts);
                    arrayOfPosts.sort((a, b) => (a.date > b.date ? -1 : 1));
                    arrayOfPosts.splice(maxAmountOfPosts);
                }
                addDetailsToComments(arrayOfPosts).then((posts) => {
                    resolve(posts);
                });
            });
        });
    ...
    Promise.all([myBestiePosts, myFriendsPosts]).then(() => {
        return res.statusJson(200, { posts, bestiePosts });
    });
};
...
const makeFriendRequest = function ({ params }, res) {
    User.findById(params.to, (err, user) => {
        ...
        user.save((err, user) => {
            ...
            return res.statusJson(201, 'Successfully sent friend request');
        });
    });
};
...
const getUserData = function ({ params }, res) {
    User.findById(params.userId, '-salt -password', { lean: true }, (err, user) => {
        ...
        if (!user) {
            res.statusJson(404, { message: 'User does not exist.' });
        }

        let generateRandomFriends = function (friendsList) {
            let copyOfFriendsList = Array.from(friendsList);
            let randomIds = [];

            for (let i = 0; i < 6; i++) {
                if (copyOfFriendsList.length <= 6) {
        ...
        let addMessengerDetails = function (messages) {
            return new Promise((resolve, reject) => {
                if (!messages.length) {
                    resolve(messages);
                }
        ...
        let randomFriends = generateRandomFriends(user.friends);
        let commentDetails = addDetailsToComments(user.posts);
        let messengerDetails = addMessengerDetails(user.messages);
        addToPosts(user.posts, user);
        ...
        Promise.all([randomFriends, commentDetails, messengerDetails, besties, enemies]).then((val) => {
...
const resolveFriendRequest = function ({ query, params }, res) {
    User.findById(params.to, (err, user) => {
        ...
        for (let i = 0; i < user.friend_requests.length; i++) {
            if (params.from === user.friend_requests[i]) {
        ...
        let promise = new Promise((resolve, reject) => {
            if (query.resolution === 'accept') {
                if (containsDuplicate([params.from, ...user.friends])) {
                    console.log(user);
                    return res.json({ message: 'Duplicate request error' });
                }
                user.friends.push(params.from);

                User.findById(params.from, function (err, user) {
                    if (err) {
                        return res.json({ error: err });
                    }
        ...
        promise.then(() => {
            user.save((err, user) => {
                ...
                alertUser(user, params.from, 'new_friend').then(() => {
                    res.statusJson(201, { message: `Resolved friend request of ${params.from}` });
                });
            });
        });
    });
...
const createPost = function ({ body, payload }, res) {
    if (!body.content || !body.theme) {
        return res.statusJson('400', { message: 'No content or theme was provided.' });
    }
...
let likeUnlikePost = function ({ payload, params }, res) {
    User.findById(params.ownerId, (err, user) => {
        ...
        promise.then(() => {
            user.save((err, user) => {
                ...
                res.statusJson(200, { message: 'Liked or unliked the post' });
            });
        });
    });
};

let postCommentOnPost = function ({ body, payload, params }, res) {
    User.findById(params.ownerId, (err, user) => {
        if (err) {
            return res.json({ err: err });
        }
        ...
        user.save((err, user) => {
            ...
            User.findById(payload._id, (err, user) => {
                ...
                promise.then(() => {
                    return res.statusJson(201, { message: 'Comment on post', comment: comment, commenter: commenter });
                });
            });
        });
    });
...
let sendMessage = function ({ body, payload, params }, res) {
    ...
    let sendMessagePromise = Promise.all([fromPromise, toPromise]).then(() => {
        ...
        return new Promise(function (resolve, reject) {
            Promise.all([sendMessageToRecipient, sendMessageToAuthor]).then((user) => {
                resolve(user);
            });
        });
    });

    sendMessagePromise.then((val) => {
        return res.statusJson(201, { message: 'Sending Message', user: val });
    });
};
