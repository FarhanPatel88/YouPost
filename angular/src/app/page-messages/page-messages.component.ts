...
export class PageMessagesComponent implements OnInit {
    constructor(private title: Title, private apiService: ApiService, private events: EventEmittersService, private cdref: ChangeDetectorRef) {}
    ...
    ngOnInit(): void {
        ...
        let userDataEvent = this.events.getUserData.subscribe((user) => {
            ...
            this.activeMessage.fromId = this.activeMessage.fromId || user.messages[user.messages.length - 1].from_id;
    ...
    public activeMessage: { fromId: string; fromName: string; fromProfilePicture: string; messageGroups: Array<any> } = {
        fromId: '',
        fromName: '',
        fromProfilePicture: '',
        messageGroups: [],
    ...
    public setActiveMessage(id: any) {
        for (let message of this.messages) {
            if (message.from_id == id) {
                ...
                let groups: Array<any> = (this.activeMessage.messageGroups = []);
    ...
    public sendMessage() {
        ...
        this.apiService.sendMessageRequest(obj, false)?.then((val: any) => {
